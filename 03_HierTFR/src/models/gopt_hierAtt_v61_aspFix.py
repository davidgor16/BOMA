...
# attention part is borrowed from espnet (https://github.com/espnet/espnet).
...
class GOPT_hierAtt(nn.Module):
    ...
    def __init__(self, embed_dim, num_heads, depth, input_dim=84):
        ...
        # Sinusoidal or learnable positional embeddings: 50 sequence positions plus 5 utterance tokens.
        ...
        self.aspect_res4_word1 = nn.Linear(embed_dim, embed_dim)
        ...
        self.aspect_res4_word2 = nn.Linear(embed_dim, embed_dim)
        ...
        self.aspect_res4_word3 = nn.Linear(embed_dim, embed_dim)
        ...
        self.aspect_res3_utt1 = nn.Linear(embed_dim, embed_dim)
        ...
        self.aspect_res3_utt2 = nn.Linear(embed_dim, embed_dim)
        self.aspect_res4_utt2 = nn.Linear(self.input_dim, embed_dim)
        ...
        self.aspect_res3_utt3 = nn.Linear(embed_dim, embed_dim)
        ...
        self.aspect_res3_utt4 = nn.Linear(embed_dim, embed_dim)
        ...
        self.aspect_res3_utt5 = nn.Linear(embed_dim, embed_dim)
        ...
        self.utt_sem1 = nn.Linear(embed_dim, 1)
        self.utt_sem2 = nn.Linear(embed_dim, 1)
        self.utt_sem3 = nn.Linear(embed_dim, 1)
        self.utt_sem4 = nn.Linear(embed_dim, 1)
        self.utt_sem5 = nn.Linear(embed_dim, 1)
    ...
    def forward(self, x, phn, word, word_pos):
        ...
        if self.embed_dim != self.input_dim:
            gop_feats = x
            x = self.in_proj(x)
            word_gop = self.in_word_proj(gop_feats)
            utt_gop = self.in_utt_proj(gop_feats)
        x = x + phn_embed
        ...
        word_feats_phn = self.word_input_att1(x[:, 5:], x[:, 5:], x[:, 5:], phn2word_msk)

        # word-level input feats: phn_tfr_feats + phn_aspsect_feats + word_id_embeds + word_pos_embeds
        
        # phn_feats + phn_aspect_feats + word_embed + gop_feats
        ...
        word_feats[:, 5:] = word_feats[:, 5:] + self.word_pos_embed(word_pos.int()+1)
        ...
        word_asp_mask = torch.zeros(B, 50, 150).to(phn.device)
        #[w_asp1, w_asp2, w_asp3]
        for l in range(tgt_msk.size(1)):
            word_asp_mask[:,l,0+l], word_asp_mask[:,l, 50+l], word_asp_mask[:,l,100+l]= tgt_msk[:, l], tgt_msk[:, l], tgt_msk[:, l]
        ...
        # prepare utt-level feats
        #utt_msk = (~(phn==-1)).int()
        ## creat aspect atten mask
        utt_msk = torch.zeros(B, 50, 50).to(phn.device)
        for idx, value in enumerate(phn):
            utt_msk[idx] = (~(value==-1)).int()

        pooling_msk = (phn != -1).unsqueeze(-1)
        # phn-utt atten
        ...
        gop_ipt = gop_feats
        gop_feats = gop_feats * pooling_msk
        gop_feats = torch.sum(gop_feats, dim=1)/pooling_msk.sum(dim=1)

        utt_feats = self.utt_input_ln(utt_gop) + self.utt_input_ln1(utt_wordFeat) + self.utt_input_ln2(utt_phnAsp) + self.utt_input_ln3(gop_feats)
        ...
        u1_feats_aspect_att = self.asp_att_u1(u1_asp_att.unsqueeze(1), asp_collect, asp_collect, None).squeeze(1)
        u2_feats_aspect_att = self.asp_att_u2(u2_asp_att.unsqueeze(1), asp_collect, asp_collect, None).squeeze(1)
        u3_feats_aspect_att = self.asp_att_u3(u3_asp_att.unsqueeze(1), asp_collect, asp_collect, None).squeeze(1)
        u4_feats_aspect_att = self.asp_att_u4(u4_asp_att.unsqueeze(1), asp_collect, asp_collect, None).squeeze(1)
        u5_feats_aspect_att = self.asp_att_u5(u5_asp_att.unsqueeze(1), asp_collect, asp_collect, None).squeeze(1)
        ...
        u1 = self.mlp_head_utt1(u1_feat + self.aspect_res2_utt1(phn_u1) + self.aspect_res3_utt1(x[:, 5:].sum(1)))
        u2 = self.mlp_head_utt2(u2_feat + self.aspect_res2_utt2(phn_u2) + self.aspect_res3_utt2(x[:, 5:].sum(1)))
        u3 = self.mlp_head_utt3(u3_feat + self.aspect_res2_utt3(phn_u3) + self.aspect_res3_utt3(x[:, 5:].sum(1)))
        u4 = self.mlp_head_utt4(u4_feat + self.aspect_res2_utt4(phn_u4) + self.aspect_res3_utt4(x[:, 5:].sum(1)))
        u5 = self.mlp_head_utt5(u5_feat + self.aspect_res2_utt5(phn_u5) + self.aspect_res3_utt5(x[:, 5:].sum(1)))
