...
parser.add_argument("--loss_w_pcc", type=float, default=1, help="weight for word-level loss")
...
def train(audio_model, train_loader, test_loader, args):
    ...
    while epoch < args.n_epochs:
        ...
        for i, (audio_input, audio_input_eng, audio_input_dur, phn_label, phns, utt_label, word_label, words) in enumerate(train_loader):
            ...
            #print(phns.shape)
            u1, u2, u3, u4, u5, p, w1, w2, w3 = audio_model(audio_input, phns, words, word_label[:,:,-1])
            ...
            #prepare for calculate pcc
            #p: [B, L] -> [B, L, 1], utt_preds: [B, 5] -> [B, L, 5], word_pred: [B, L, 3]
            p = p.unsqueeze(-1)
            phn_label = phn_label.unsqueeze(-1)

            utt_preds = utt_preds.unsqueeze(1).repeat(1,p.size(1),1)
            utt_label = utt_label.unsqueeze(1).repeat(1,p.size(1),1)

            xs_predi = torch.cat([p, utt_preds, word_pred], dim=-1)
            ys_label = torch.cat([phn_label, utt_label, word_label], dim=-1)

            # 9: phn1+word3+utt5
            noise = torch.rand(ys_label.shape) * 1e-6
            noise = noise.to(device, non_blocking=True)
            # prevent nan/ zero std when calculate pcc
            xs_predi = xs_predi + noise
            ys_label = ys_label + noise

            loss_pcc_sum = 0

            for batch in range(ys_label.size(0)):
                pcc_xs_predi = torch.corrcoef(xs_predi[batch].t())
                pcc_ys_label = torch.corrcoef(ys_label[batch].t())
                loss_pcc = loss_fn_pcc(pcc_xs_predi, pcc_ys_label)
                msk = torch.ones(9, 9).to(device)
                msk = torch.triu(msk).bool()
                loss_pcc = (loss_pcc * msk).sum() / torch.sum(msk)
                loss_pcc_sum = loss_pcc_sum + loss_pcc
            loss_pcc_sum = loss_pcc_sum / ys_label.size(0)
            #loss_pcc_sum = loss_pcc_sum

            loss = args.loss_w_phn * loss_phn + args.loss_w_utt * loss_utt + args.loss_w_word * loss_word
            loss = loss + args.loss_w_pcc * loss_pcc_sum
        ...
        print("ending one epoch training:(%s)s" % (int(round(time.time())-s_t)))
        ...
        avg_mse = np.mean(te_mse) + np.mean(te_utt_mse) + np.mean(te_word_mse)
        ...
        print('Loss-MSE:, phn: {:.3f}, word: {:.3f}, utt: {:.3f}, pcc: {:.3f}'.format(loss_phn, loss_word, loss_utt, loss_pcc))
        ...
        print("ending validation:(%s)s" % (int(round(time.time())-s_t)))
...
def validate(audio_model, val_loader, args, best_mse):
    ...
    with torch.no_grad():
        for i, (audio_input, audio_input_eng, audio_input_dur, phn_label, phns, utt_label, word_label, words) in enumerate(val_loader):
        ...
        avg_mse = np.mean(phn_mse) + np.mean(word_mse) + np.mean(utt_mse)
        if phn_mse < best_mse:
...
if __name__ == '__main__':
    ...
    if args.model == 'gopt':
        ...
        from models.gopt_hierAtt_v61_aspFix import GOPT_hierAtt
        audio_mdl = GOPT_hierAtt(embed_dim=args.embed_dim, num_heads=args.goptheads, depth=args.goptdepth, input_dim=input_dim)
        gopt_path_init="../exp_hierTFR/v6_aspFix_preMdl/v61_v1/0/models/best_audio_model.pth"
        state_dict = torch.load(gopt_path_init, map_location=torch.device('cpu'))
        new_state_dict = OrderedDict()
        for k, v in state_dict.items():
            name = k[7:] # remove `module.`
            new_state_dict[name] = v
        audio_mdl.load_state_dict(new_state_dict, strict=False)
