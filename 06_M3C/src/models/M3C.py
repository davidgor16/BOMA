...
class M3C(nn.Module):
    def __init__(self,
        ...
        self.lpp_vowels_indices = [0, 1, 2, 3, 4, 5, 10, 11, 12, 16, 17, 24, 25, 32, 33]
        self.lpp_consonants_indices = [6, 7, 8, 9, 13, 14, 15, 18, 19, 20, 21, 22, 23, 26, 27, 28, 29, 30, 31, 34, 35, 36, 37, 38]
        
        self.lpp_vowels_mean = 5.511906481828478
        self.lpp_vowels_std =  0.6837013186493792
        self.lpr_vowels_mean = 0.8308196800273836
        self.lpr_vowels_std =  0.8265027665274315
        self.ohe_vowels_mean = 0.06666666666666667
        self.ohe_vowels_std = 0.24944382578492946

        self.lpp_consonants_mean = 4.992626953058629
        self.lpp_consonants_std = 0.7323012711549867
        self.lpr_consonants_mean = 1.4572945658514183
        self.lpr_consonants_std = 0.9923422506840686
        self.ohe_consonants_mean = 0.041666666666666664
        self.ohe_consonants_std = 0.19982631347136334
    ...
    def forward(self, x, phn, words, dur_feat, ener_feat,w2v_feat,hubert_feat,wavlm_feat):
        ...
        # Separate LPP and LPR features.
        x_lpp = x[:,:,:42]
        x_lpr = x[:,:,42:]
        
        # Discard the first three positions, which represent non-phoneme symbols.
        x_lpp = x_lpp[:,:,3:]
        x_lpr = x_lpr[:,:,3:]
        
        # Create a one-hot vector whose active position has LPR equal to zero.
        lpr_zero_mask = (x_lpr == 0).float()
        ohe = torch.zeros_like(x_lpr)
        ohe[lpr_zero_mask == 1] = 1
        
        # Build a matrix by concatenating LPP, LPR, and the one-hot encoding.
        x = torch.stack((x_lpp, x_lpr, ohe), dim=2)
        ...
        for i in range(x.shape[0]):
            ...
            # GOP
            x_vowels = x[i,index_vowels[i] != -1]
            x_vowels = x_vowels[:,:, self.lpp_vowels_indices]
            
            x_consonants = x[i,index_consonants[i] != -1]
            x_consonants = x_consonants[:,:, self.lpp_consonants_indices]
        
            # Normalize vowel and consonant features separately.
            x_vowels = self.vowel_normalization(x_vowels)
            x_consonants = self.consonant_normalization(x_consonants)
    ...
    def vowel_normalization(self, x_vowels):
        x_vowels[:,0] = (x_vowels[:,0] - self.lpp_vowels_mean) / self.lpp_vowels_std
        x_vowels[:,1] = (x_vowels[:,1] - self.lpr_vowels_mean) / self.lpr_vowels_std
        x_vowels[:,2] = (x_vowels[:,2] - self.ohe_vowels_mean) / self.ohe_vowels_std
        return x_vowels
        
    
    def consonant_normalization(self, x_consonants):
        x_consonants[:,0] = (x_consonants[:,0] - self.lpp_consonants_mean) / self.lpp_consonants_std
        x_consonants[:,1] = (x_consonants[:,1] - self.lpr_consonants_mean) / self.lpr_consonants_std
        x_consonants[:,2] = (x_consonants[:,2] - self.ohe_consonants_mean) / self.ohe_consonants_std
        return x_consonants
